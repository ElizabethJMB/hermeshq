...
MIG_PATH = "hermeshq.alembic.versions.b1c2d3e4f5a6_add_check_constraints_for_status_columns"
...
class TestConstraintExists:
    def test_returns_true_when_found(self):
        ...
        with patch(f"{MIG_PATH}.op", mock_op), patch(f"{MIG_PATH}.inspect", mock_inspect):
...
class TestUpgrade:
    def test_creates_both_constraints_when_missing(self):
        ...
        with patch(f"{MIG_PATH}.op", mock_op), patch.object(mod, "_constraint_exists", return_value=False):
            mod.upgrade()
    ...
    def test_skips_agents_constraint_when_exists(self):
        ...
        with patch(f"{MIG_PATH}.op", mock_op), patch.object(mod, "_constraint_exists", side_effect=side_effect):
            mod.upgrade()
    ...
    def test_skips_all_when_both_exist(self):
        ...
        with patch(f"{MIG_PATH}.op", mock_op), patch.object(mod, "_constraint_exists", return_value=True):
            mod.upgrade()
    ...
    def test_agents_constraint_values(self):
        ...
        agents_calls = [c for c in mock_op.create_check_constraint.call_args_list if c[0][0] == "ck_agents_status"]
    ...
    def test_tasks_constraint_values(self):
        ...
        tasks_calls = [c for c in mock_op.create_check_constraint.call_args_list if c[0][0] == "ck_tasks_status"]
