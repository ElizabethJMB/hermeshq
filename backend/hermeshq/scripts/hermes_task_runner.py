...
def _snapshot_directory(directory: Path) -> dict[str, dict]:
    ...
    Skips dot-directories (.hermes, .git, etc.), uploads/, and other
    infrastructure directories that should not be treated as agent output.
    """
    snapshot: dict[str, dict] = {}
    if not directory.exists():
        return snapshot
    _SKIP_DIRS = {".hermes", ".git", "__pycache__", "uploads", "node_modules", ".venv", "venv"}
    for path in directory.rglob("*"):
        # Skip files inside infrastructure directories
...
def _collect_response_attachments(
    cwd: Path, pre_workspace: dict, pre_tmp: dict, *, tmp_dir: Path | None = None,
) -> list[dict]:
    """Scan workspace and /tmp for new/modified files since snapshots,
    copy allowed files to uploads/, and return metadata list suitable
    for the result payload.

    Scans:
      - cwd (workspace root) excluding .hermes/, uploads/, etc.
      - /tmp (system temp) — agents commonly write here
    """
    if tmp_dir is None:
        tmp_dir = Path("/tmp")
    ...
    # ── Scan workspace ──
    post_workspace = _snapshot_directory(cwd)
    diff_ws = _diff_snapshots(pre_workspace, post_workspace)

    # ── Scan tmp_dir ──
    post_tmp = _snapshot_directory(tmp_dir)
    diff_tmp = _diff_snapshots(pre_tmp, post_tmp)

    # Combine diffs
    all_diff = diff_ws + diff_tmp
...
def main() -> int:
    ...
    # ── Snapshot BEFORE execution (for response attachment detection) ──
    _pre_work_snapshot = _snapshot_directory(Path(payload["cwd"]))
    _pre_tmp_snapshot = _snapshot_directory(Path("/tmp"))
    ...
    try:
        ...
        response_attachments = _collect_response_attachments(
            Path(payload["cwd"]), _pre_work_snapshot, _pre_tmp_snapshot
        )
