...
def _collect_response_attachments(cwd: Path, pre_workspace: dict, *, tmp_dir: Path | None = None) -> list[dict]:
    """Scan workspace for new/modified files since snapshot,
    copy allowed files to uploads/, and return metadata list suitable
    for the result payload.

    Only scans the workspace root (excluding .hermes/, uploads/, etc.).
    Files in /tmp or other system locations are handled by the MEDIA: parser
    in hermes_runtime.py.
    """
...
def main() -> int:
    ...
    # Only snapshot the workspace — /tmp is handled by the MEDIA: parser
    # in hermes_runtime.py which is safer and more targeted.
